...
class UserManager extends AbstractManager {
  ...
  async findByEmail(email) {
    const user = await this.connection.query(
      `SELECT u.id, u.username, u.email, u.role, u.creationDate, u.current_location_id, p.id planetId FROM ${UserManager.table} u LEFT JOIN planet p ON p.userId=u.id WHERE u.email = ?`,
      [email]
    );
    return user[0];
  }
  ...
  async findById(id) {
    const user = await this.connection.query(
      `SELECT u.id, u.username, u.email, u.role, u.creationDate, u.current_location_id, p.id planetId FROM ${UserManager.table} u LEFT JOIN planet p ON p.userId=u.id WHERE u.id = ?`,
      [id]
    );
    return user[0];
  }
